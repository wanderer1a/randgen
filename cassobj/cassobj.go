...
func SessionInit(seeds string) *gocql.Session {
	...
	cluster.WriteTimeout = 30000
	cluster.Consistency = gocql.Quorum
	cluster.DisableInitialHostLookup = true
	//cluster.ConnectTimeout = 600
	//cluster.Timeout = 600
	var err error
...
func PanInsert(num int, now string, pan int, pan_hash string, seeds string) {
	session := SessionInit(seeds)
	...
	if err := session.Query(`INSERT INTO generator.card_pan_sha256
							...
							(?, ?, ?, ?)`,
		num, now, pan, pan_hash).Exec(); err != nil {
...
func TrxInsert(trx, seeds string) {
	session := SessionInit(seeds)
	fmt.Print("Ready to pan and hash insertion...")
	if err := session.Query(`INSERT INTO generator.card_pan_sha256
							(
